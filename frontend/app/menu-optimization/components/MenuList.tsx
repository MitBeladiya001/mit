...
import { Loader2, Trash2, Sparkles, ChefHat, DollarSign, Plus } from "lucide-react";
...
export function MenuList() {
  ...
  const handleDeleteMenu = async (menuId: string) => {
    try {
      const response = await axios.delete(`http://localhost:5001/menus/${menuId}`);
      if (response.data.success) {
  ...
  return (
    <div className="space-y-8">
      <div className="flex justify-between items-center">
        ...
        <Link href="/menu-optimization/CreateMenuForm">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {menus.map((menu) => (
          <Card key={`menu-${menu._id}`} className="bg-gray-900 border-gray-800">
        ...
        <div className="space-y-8 mt-8">
          ...
          {optimizationResults.daily_specials.length > 0 && (
            <Card className="bg-gray-900 border-gray-800">
              ...
              <CardContent>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {optimizationResults.daily_specials.map((item: any, index: number) => (
                    <div
                      key={index}
            ...
            <Card className="bg-gray-900 border-gray-800">
              ...
              <CardContent>
                <div className="space-y-4">
                  {optimizationResults.cost_optimizations.map((item: any, index: number) => (
                    <div
                      key={index}
                      className="bg-gray-800/50 rounded-lg p-4 space-y-2"
                    >
          ...
          {optimizationResults.new_dishes.length > 0 && (
            <Card className="bg-gray-900 border-gray-800">
              ...
              <CardContent>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {optimizationResults.new_dishes.map((item: any, index: number) => (
                    <div
                      key={index}
                      className="bg-gray-800/50 rounded-lg p-4 space-y-2"
                    >
